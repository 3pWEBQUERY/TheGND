...
export default function AgencyPageClient(props: {
  ...
  return (
    <Suspense fallback={<div className="min-h-screen bg-white"><MinimalistNavigation /><div className="max-w-5xl mx-auto px-6 py-10">Laden...</div><Footer /></div>}>
      <AgencyPageInner {...props} />
    </Suspense>
  )
}

function AgencyPageInner({ initialItems, initialTotal, initialQ, initialLocation, initialSort }: {
  initialItems: AgencyItem[]
  initialTotal: number
  initialQ: string
  initialLocation: string
  ...
  async function fetchAgencies(opts?: { append?: boolean; skip?: number; take?: number; sort?: string }) {
    setLoading(true)
    setError(null)
    try {
      // Keep the URL in sync for q/location only
      const urlParams = new URLSearchParams()
      if (q.trim()) urlParams.set('q', q.trim())
      if (location.trim()) urlParams.set('location', location.trim())
      router.replace(`/agency?${urlParams.toString()}`)

      // Build request params using overrides when provided
      ...
      const res = await fetch(`/api/agency/search?${reqParams.toString()}`, { cache: 'no-store' })
  ...
  // Initialize from URL if present, otherwise rely on initial props. Do not refetch on mount if props provided.
  useEffect(() => {
  ...
  // Debounced search when q/location/sort changes
  useEffect(() => {
    if (!initializedRef.current) return
    const t = setTimeout(() => {
      setSkip(0)
      fetchAgencies({ append: false, skip: 0, take, sort })
    }, 300)
  ...
  async function loadMore() {
    ...
    await fetchAgencies({ append: true, skip: nextSkip, take, sort })
    setSkip(nextSkip)
  }

  // Infinite scroll via IntersectionObserver (keeps button as fallback)
