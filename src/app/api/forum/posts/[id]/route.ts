...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const post = await prisma.forumPost.findUnique({ where: { id: params.id }, select: { authorId: true, thread: { select: { forum: { select: { isLocked: true } }, isClosed: true } } } })
    ...
    await prisma.forumPost.update({ where: { id: params.id }, data: { content, editedAt: new Date() } })
