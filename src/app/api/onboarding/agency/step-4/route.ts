...
import { businessOnboardingStep4Schema } from '@/lib/validations'
...
export async function GET() {
  try {
    ...
    return NextResponse.json({ logo, gallery, media }, { status: 200 })
  } catch (error) {
    console.error('Agency step 4 GET error:', error)
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = businessOnboardingStep4Schema.safeParse(body)
    if (!result.success) {
    ...
    console.error('Agency step 4 POST error:', error)
