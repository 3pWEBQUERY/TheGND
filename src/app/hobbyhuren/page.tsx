...
import HobbyhurenHero from '@/components/homepage/HobbyhurenHero'
import HobbyhurenSearch from '@/components/hobbyhuren/HobbyhurenSearch'
...
export default function HobbyhurenPage() {
  return (
...
function HobbyhurenPageInner() {
  ...
  async function fetchHobbyhuren() {
    setLoading(true)
    setError(null)
    try {
      const urlParams = new URLSearchParams(queryString)
      if (viewMode !== 'grid') urlParams.set('view', viewMode)
      router.replace(`/hobbyhuren?${urlParams.toString()}`)
      const res = await fetch(`/api/hobbyhuren/search?${queryString}`, { cache: 'no-store' })
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      setError(null)
      try {
        const res = await fetch(`/api/hobbyhuren/search?${p.toString()}`, { cache: 'no-store' })
        if (!res.ok) throw new Error('Fehler beim Laden')
        const data = await res.json()
        setItems(data.items)
        setTotal(data.total)
  ...
  useEffect(() => {
    ...
    router.replace(`/hobbyhuren?${baseParams.toString()}`)
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [viewMode])
