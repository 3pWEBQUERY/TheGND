...
export default function ClubOnboardingStep5() {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        const res = await fetch('/api/onboarding/club/step-5')
        if (!res.ok) return
  ...
  const onSubmit = async (data: Step5Form) => {
    ...
    try {
      ...
      router.push(addEditParam('/onboarding/club/step-6'))
    } catch (e) {
              ...
              <Controller
                ...
                render={({ field }) => (
                  <MultiSelect
                    ...
                    placeholder="Wähle, was euer Club bietet (z. B. Darkroom, Sauna, Raucherbereich)"
