...
const SOCIAL_PRESETS: { value: string; label: string; placeholder?: string }[] = [
  ...
  { value: 'website', label: 'Club-Website', placeholder: 'https://example.com' },
]
...
export default function ClubOnboardingStep6() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const res = await fetch('/api/onboarding/club/step-6')
        if (!res.ok) return
  ...
  const onSubmit = async (data: Step6Form) => {
    ...
    try {
      ...
      router.push(addEditParam('/onboarding/club/step-7'))
    } catch (e: any) {
      ...
      <nav className="absolute top-0 w-full z-50 bg-transparent">
        <div className="max-w-7xl mx-auto px-6 py-6">
          <div className="flex justify-between items-center">
            ...
            <div className="text-sm font-light tracking-widest text-gray-600">CLUB-EINRICHTUNG – SCHRITT 6/7</div>
          </div>
        </div>
      </nav>
        ...
        <div className="w-full max-w-2xl">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
            ...
            <div>
              ...
              <Input id="website" placeholder="https://mein-club.de" className="rounded-none mt-2" {...register('website', { setValueAs: (v) => (typeof v === 'string' && v.trim() === '' ? undefined : v) })} />
