...
type Step6Form = z.infer<typeof escortOnboardingStep6Schema>
...
const SOCIAL_PRESETS: { value: string; label: string; placeholder?: string }[] = [
  ...
  { value: 'website', label: 'Persönliche Website', placeholder: 'https://example.com/me' },
]
...
function platformIcon(name: string) {
  ...
  switch (n) {
    ...
    case 'snapchat':
      return <SiSnapchat className="h-4 w-4" />
    case 'onlyfans':
      return <SiOnlyfans className="h-4 w-4" />
    default:
...
export default function EscortOnboardingStep6() {
  ...
  const { register, handleSubmit, formState: { errors }, reset } = useForm<Step6Form>({
    resolver: zodResolver(escortOnboardingStep6Schema),
    defaultValues: {
      phone: '',
      website: undefined
    }
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        // Ignorieren – Vorbefüllung ist optional
      }
    }
    load()
    return () => { active = false }
  ...
  const addSocial = () => {
    ...
    if (newPlatform === 'custom' && platformKey.length < 2) {
      setError('Bitte gib einen Namen für die Plattform an (mind. 2 Zeichen).')
      return
    }
    let valueToStore = newUrl
    const isOverride = socials.some(s => s.platform === platformKey)
    if (!isOverride && socials.length >= 10) {
      setError('Maximal 10 Social-Links erlaubt.')
      return
    }
    if (platformKey.toLowerCase() === 'whatsapp') {
      const cleaned = cleanPhone(newUrl)
      if (!phoneRegex.test(cleaned)) {
        setError('WhatsApp erfordert eine gültige Telefonnummer (z. B. +491701234567), keine URL.')
        return
      }
      valueToStore = cleaned
    } else {
      try {
        // URL-Validierung für alle nicht-WhatsApp Plattformen
        // eslint-disable-next-line no-new
        new URL(newUrl)
      } catch {
        setError('Bitte gib eine gültige URL für den Social-Link ein.')
        return
      }
    }
    setError('')
    setSocials(prev => {
      ...
      if (exists) {
        // Überschreiben, um Duplikate zu vermeiden
        return prev.map(s => s.platform === platformKey ? { platform: platformKey, url: valueToStore } : s)
      }
  ...
  const onSubmit = async (data: Step6Form) => {
    ...
    setError('')
    try {
      const socialRecord: Record<string, string> | undefined = socials.length
        ? Object.fromEntries(socials.map(s => [s.platform, s.url]))
        : undefined
      ...
      const res = await fetch('/api/onboarding/escort/step-6', {
        ...
        body: JSON.stringify({ phone, website, socialMedia: socialRecord })
      })
        ...
        <div className="max-w-7xl mx-auto px-6 py-6">
          <div className="flex justify-between items-center">
            ...
            <div className="text-sm font-light tracking-widest text-gray-600">
              ESCORT-EINRICHTUNG – SCHRITT 6/7
            </div>
          </div>
        </div>
      </nav>
        ...
        <div className="w-full max-w-2xl">
          ...
          <div className="text-center mb-16">
            ...
            <p className="text-lg font-light tracking-wide text-gray-600 max-w-md mx-auto mb-4">
              Telefon, Website & Social – Schritt 6 von 7
            </p>
            <div className="flex justify-center">
              <Badge className="bg-pink-500 text-white font-light tracking-widest px-4 py-1 rounded-none">SCHRITT 6/7</Badge>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
            ...
            <div>
              ...
              <Input
                ...
                {...register('website', { setValueAs: (v) => (typeof v === 'string' && v.trim() === '' ? undefined : v) })}
              />
              ...
              <div className="mt-3 flex flex-col gap-3 border p-3 bg-gray-50">
                ...
                <div className="flex flex-col sm:flex-row gap-3">
                  <Select value={newPlatform} onValueChange={setNewPlatform}>
                    <SelectTrigger className="rounded-none flex-[0.7]">
                  ...
                  {newPlatform === 'custom' && (
                    <Input
                      placeholder="Plattformname (z.B. WeChat, Line, VK)"
                  ...
                  <Input
                    placeholder={
                      newPlatform === 'whatsapp'
                        ? '+491701234567'
                        : SOCIAL_PRESETS.find(p => p.value === newPlatform)?.placeholder || 'https://...'
                    }
                    value={newUrl}
                    onChange={(e) => setNewUrl(e.target.value)}
                    className="rounded-none flex-1"
                  />
                  <Button type="button" onClick={addSocial} className="rounded-none bg-pink-500 hover:bg-pink-600 text-white">
                  ...
                  <div className="flex flex-col divide-y">
                    {socials.map((s) => (
                      <div key={s.platform} className="flex items-center gap-3 py-2">
                        ...
                        <Button type="button" variant="outline" onClick={() => removeSocial(s.platform)} className="rounded-none border-gray-300 text-gray-600 hover:text-pink-600 hover:border-pink-600">
                ...
                {socials.length === 0 && (
                  <p className="text-xs text-gray-500">Noch keine Social-Links hinzugefügt.</p>
                )}
              </div>
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-6 pt-4">
              ...
              >
                {isLoading ? 'Speichern ...' : 'Weiter zu Schritt 7'}
              </Button>
            </div>
          </form>
        </div>
