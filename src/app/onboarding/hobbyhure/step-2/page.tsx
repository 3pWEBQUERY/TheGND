...
type Step2Form = z.infer<typeof escortOnboardingStep2Schema>
...
export default function HobbyhureOnboardingStep2() {
  ...
  const { register, handleSubmit, control, reset, formState: { errors } } = useForm<Step2Form>({
    resolver: zodResolver(escortOnboardingStep2Schema),
    defaultValues: { piercings: [], tattoos: [] }
  })
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        const res = await fetch('/api/onboarding/hobbyhure/step-2')
        if (!res.ok) return
        const data = await res.json()
        if (!active || !data) return
        const defaults: Partial<Step2Form> = {
  ...
  const onSubmit = async (data: Step2Form) => {
    ...
    setError('')
    try {
      const response = await fetch('/api/onboarding/hobbyhure/step-2', {
        method: 'POST',
      ...
      router.push(addEditParam('/onboarding/hobbyhure/step-3'))
    } catch (e) {
        ...
        <div className="max-w-7xl mx-auto px-6 py-6">
          <div className="flex justify-between items-center">
            ...
            <div className="text-sm font-light tracking-widest text-gray-600">
              HOBBYHURE-EINRICHTUNG – SCHRITT 2/7
            </div>
          </div>
        </div>
      </nav>
        ...
        <div className="w-full max-w-2xl">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-12">
            ...
            <div className="space-y-8">
              <div className="grid md:grid-cols-2 gap-8">
                <div className="space-y-3">
                  ...
                  <Input id="height" placeholder="z. B. 170" {...register('height')} className={`border-0 border-b-2 border-gray-200 rounded-none px-0 py-3 text-sm font-light focus:border-pink-500 focus:ring-0 bg-transparent ${errors.height ? 'border-red-500' : ''}`} />
                </div>
                <div className="space-y-3">
                  <Label className="text-xs font-light tracking-widest text-gray-800 uppercase">Gewicht (kg)</Label>
                  <Input id="weight" placeholder="z. B. 55" {...register('weight')} className={`border-0 border-b-2 border-gray-200 rounded-none px-0 py-3 text-sm font-light focus:border-pink-500 focus:ring-0 bg-transparent ${errors.weight ? 'border-red-500' : ''}`} />
                </div>
              </div>
              ...
              <div className="grid md:grid-cols-3 gap-8">
                  ...
                  <Controller name="bodyType" control={control} render={({ field }) => (
                    <Select onValueChange={field.onChange} value={field.value}>
                      ...
                      <SelectContent className="rounded-none">
                        {bodyTypeOptions.map((o) => (<SelectItem className="rounded-none" key={o.value} value={o.value}>{o.label}</SelectItem>))}
                      </SelectContent>
                    </Select>
                  )} />
                </div>
                <div className="space-y-3">
                  ...
                  <Controller name="hairColor" control={control} render={({ field }) => (
                    <Select onValueChange={field.onChange} value={field.value}>
                      ...
                      <SelectContent className="rounded-none">
                        {hairColorOptions.map((o) => (<SelectItem className="rounded-none" key={o.value} value={o.value}>{o.label}</SelectItem>))}
                      </SelectContent>
                    </Select>
                  )} />
                </div>
                <div className="space-y-3">
                  ...
                  <Controller name="hairLength" control={control} render={({ field }) => (
                    <Select onValueChange={field.onChange} value={field.value}>
                      ...
                      <SelectContent className="rounded-none">
                        {hairLengthOptions.map((o) => (<SelectItem className="rounded-none" key={o.value} value={o.value}>{o.label}</SelectItem>))}
                      </SelectContent>
                    </Select>
                  )} />
                </div>
                ...
                <div className="space-y-3">
                  ...
                  <Controller name="eyeColor" control={control} render={({ field }) => (
                    <Select onValueChange={field.onChange} value={field.value}>
                      ...
                      <SelectContent className="rounded-none">
                        {eyeColorOptions.map((o) => (<SelectItem className="rounded-none" key={o.value} value={o.value}>{o.label}</SelectItem>))}
                      </SelectContent>
                    </Select>
                  )} />
                </div>
              ...
              <div className="grid md:grid-cols-3 gap-8">
                <div className="space-y-3">
                  ...
                  <Controller name="breastType" control={control} render={({ field }) => (
                    <Select onValueChange={field.onChange} value={field.value}>
                      ...
                      <SelectContent className="rounded-none">
                        {breastTypeOptions.map((o) => (<SelectItem className="rounded-none" key={o.value} value={o.value}>{o.label}</SelectItem>))}
                      </SelectContent>
                    </Select>
                  )} />
                </div>
                <div className="space-y-3">
                  ...
                  <Controller name="breastSize" control={control} render={({ field }) => (
                    <Select onValueChange={field.onChange} value={field.value}>
                      ...
                      <SelectContent className="rounded-none">
                        {breastSizeOptions.map((o) => (<SelectItem className="rounded-none" key={o.value} value={o.value}>{o.label}</SelectItem>))}
                      </SelectContent>
                    </Select>
                  )} />
                </div>
                <div className="space-y-3">
                  ...
                  <Controller name="intimateArea" control={control} render={({ field }) => (
                    <Select onValueChange={field.onChange} value={field.value}>
                      ...
                      <SelectContent className="rounded-none">
                        {intimateAreaOptions.map((o) => (<SelectItem className="rounded-none" key={o.value} value={o.value}>{o.label}</SelectItem>))}
                      </SelectContent>
                    </Select>
                  )} />
                </div>
              ...
              <div className="grid md:grid-cols-2 gap-8">
                <div className="space-y-3">
                  ...
                  <Controller name="piercings" control={control} render={({ field }) => (
                    <MultiSelect options={piercingOptions} value={field.value || []} onChange={field.onChange} placeholder="Wähle Piercing-Stellen" searchPlaceholder="Suchen..." className="w-full" />
                  )} />
                </div>
                <div className="space-y-3">
                  <Label className="text-xs font-light tracking-widest text-gray-800 uppercase">Tätowierungen</Label>
                  <Controller name="tattoos" control={control} render={({ field }) => (
                    <MultiSelect options={tattooOptions} value={field.value || []} onChange={field.onChange} placeholder="Wähle Tattoo-Bereiche" searchPlaceholder="Suchen..." className="w-full" />
                  )} />
                </div>
              </div>
              ...
              <div className="grid md:grid-cols-3 gap-8">
                <div className="space-y-3">
                  ...
                  <Controller name="clothingStyle" control={control} render={({ field }) => (
                    <Select onValueChange={field.onChange} value={field.value}>
                      ...
                      <SelectContent className="rounded-none">
                        {clothingStyleOptions.map((o) => (<SelectItem className="rounded-none" key={o.value} value={o.value}>{o.label}</SelectItem>))}
                      </SelectContent>
                    </Select>
                  )} />
                </div>
                <div className="space-y-3">
                  ...
                  <Input id="clothingSize" placeholder="z. B. S / 36" {...register('clothingSize')} className={`border-0 border-b-2 border-gray-200 rounded-none px-0 py-3 text-sm font-light focus:border-pink-500 focus:ring-0 bg-transparent ${errors.clothingSize ? 'border-red-500' : ''}`} />
                </div>
                <div className="space-y-3">
                  <Label className="text-xs font-light tracking-widest text-gray-800 uppercase">Schuhgröße (EU)</Label>
                  <Controller name="shoeSize" control={control} render={({ field }) => (
                    <Select onValueChange={field.onChange} value={field.value}>
                      ...
                      <SelectContent className="rounded-none">
                        {shoeSizeOptions.map((o) => (<SelectItem className="rounded-none" key={o.value} value={o.value}>{o.label}</SelectItem>))}
                      </SelectContent>
                    </Select>
                  )} />
                </div>
            ...
            <div className="flex flex-col sm:flex-row gap-6 pt-8">
              ...
              <Button type="submit" disabled={isLoading} className="flex-1 bg-pink-500 hover:bg-pink-600 text-white font-light tracking-widest py-4 text-sm uppercase rounded-none">{isLoading ? 'Speichern...' : 'Weiter zu Schritt 3'}</Button>
            </div>
          </form>
        </div>
      </div>
