...
type Step3Form = z.infer<typeof escortOnboardingStep3Schema>

export default function HobbyhureOnboardingStep3() {
  ...
  const { control, handleSubmit, reset, formState: { errors } } = useForm<Step3Form>({
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        const res = await fetch('/api/onboarding/hobbyhure/step-3')
        if (!res.ok) return
  ...
  const onSubmit = async (data: Step3Form) => {
    ...
    try {
      ...
      router.push(addEditParam('/onboarding/hobbyhure/step-4'))
    } catch (e) {
        ...
        <div className="max-w-7xl mx-auto px-6 py-6">
          <div className="flex justify-between items-center">
            ...
            <div className="text-sm font-light tracking-widest text-gray-600">
              HOBBYHURE-EINRICHTUNG – SCHRITT 3/7
            </div>
          </div>
        </div>
      </nav>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-12">
            ...
            <div className="space-y-3">
              ...
              <Controller name="description" control={control} render={({ field }) => (
                <RichTextEditor value={field.value || ''} onChange={field.onChange} placeholder="Beschreibe dich, deine Persönlichkeit und was dich besonders macht…" className="w-full" />
              )} />
              {errors.description && (
            ...
            <div className="flex flex-col sm:flex-row gap-6 pt-8">
              ...
              <Button type="submit" disabled={isLoading} className="flex-1 bg-pink-500 hover:bg-pink-600 text-white font-light tracking-widest py-4 text-sm uppercase rounded-none">{isLoading ? 'Speichern…' : 'Weiter zu Schritt 4'}</Button>
            </div>
          </form>
        </div>
      </div>
