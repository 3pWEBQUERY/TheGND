...
export default function HobbyhureOnboardingStep6() {
  ...
  const { register, handleSubmit, formState: { errors }, reset } = useForm<Step6Form>({
    ...
    defaultValues: { phone: '', website: undefined }
  })
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        const res = await fetch('/api/onboarding/hobbyhure/step-6')
        if (!res.ok) return
  ...
  const onSubmit = async (data: Step6Form) => {
    ...
    try {
      ...
      router.push(addEditParam('/onboarding/hobbyhure/step-7'))
    } catch (e: any) {
        ...
        <div className="max-w-7xl mx-auto px-6 py-6">
          <div className="flex justify-between items-center">
            ...
            <div className="text-sm font-light tracking-widest text-gray-600">
              HOBBYHURE-EINRICHTUNG – SCHRITT 6/7
            </div>
          </div>
        </div>
      </nav>
