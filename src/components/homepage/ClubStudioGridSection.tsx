...
export default function ClubStudioGridSection() {
  ...
  useEffect(() => {
    ;(async () => {
      try {
        const res = await fetch('/api/club-studio/search?take=12', { cache: 'no-store' })
        if (res.ok) {
  ...
  return (
    <section id="clubs-studios" className="py-20 bg-white">
      <div className="max-w-7xl mx-auto px-6">
        <div className="text-center mb-16">
          <h2 className="text-4xl font-thin tracking-wider text-gray-800 mb-4">CLUBS & STUDIOS</h2>
        ...
        <div className="text-center">
          <Button asChild variant="outline" className="text-sm font-light tracking-widest px-8 py-3 border-gray-300 hover:border-pink-500">
            <Link href="/club-studio">ALLE CLUBS & STUDIOS ANSEHEN</Link>
          </Button>
        </div>
      </div>
    </section>
